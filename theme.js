...
//config code
var company_name = "Ability Beyond";
// .header-outer height
...
$(document).ready(function() {
    ...
    $(window).resize(function(){
        ...
        // Custom Layouts: 2-Column Hexagon Media
        $('.column-media__hexagon').each(function() {
            $(this).outerHeight($(this).outerWidth() * .8333)
        })
    });
    ...
    $(document).on("click", ".cs_jsearchform .ui-autocomplete-foot a", function(e){
        window.location.href = "/search/jobs/?q="+$('#cs_jobsearchforminput_keyword').val();
    });
    ...
    // Pathways Program TN links
    $('.pathways-program-tn-link').click(function() {
        $('a#pathways-program-tn-main')[0].click();
    });

    // General Referral links
    $('.general-referral-link').click(function() {
        $('#general-referral-main a')[0].click();
    });
    ...
    // Facet code 2
    // Show/hide facets
    $(document).on('click', '.facet-filter-results-button', function() { $('.facet-section').toggleClass('active'); });
